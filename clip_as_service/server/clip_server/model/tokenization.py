...
class Tokenizer:
    def __init__(self, name: str, **kwargs):
        ...
        elif name in _SIGLIP_MODELS or "siglip" in name:
            from transformers import AutoProcessor
            self._tokenizer = AutoProcessor.from_pretrained(name).tokenizer
        else:
    ...
    def __call__(
        ...
        """
        if self._name in _SIGLIP_MODELS or "siglip" in self._name:
            return self._tokenize(texts, context_length=64)
    ...
    def _tokenize(
        ...
        if isinstance(texts, str):
            texts = [texts]
        if self._name in _SIGLIP_MODELS or "siglip" in self._name:
            result = self._tokenizer(text=texts, padding="max_length", return_tensors="pt", truncation=True, return_attention_mask=True)
            return {
                'input_ids': result['input_ids'],
                'attention_mask': result['attention_mask'],
            }
        elif self._name in _MULTILINGUALCLIP_MODELS:
