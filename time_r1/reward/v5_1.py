                ...
                if isinstance(content, dict) and content["type"] == "text":
                    ...
                    if match_answer:
                        ...
                        if type == 'open_ended':
                            ...
                            s = 1.0 if answer.strip() == ground_truth.strip() else 0.0
                        else:
                            s = llm_judge_score(answer, ground_truth, question)
                        answer_score_list.append(s)
...
def compute_tool_score(messages, ground_truth, question, completion, type):
    """
    Compute the llm judge score for a single message.
...
def tool_reward(completions, messages, target, question, type, **kwargs):
    """
    Calculate the llm judge reward.
    """
    reward_list = []
    for msg, sol, q, comp, t in zip(messages, target, question, completions, type):
        score = compute_tool_score(msg, sol["answer"], q, comp, t)
        reward_list.append(score)
    return reward_list
