...
def sequence_answer_reward(completions, target, **kwargs):
    """
    ...
    for content, sol in zip(completions, target): 
        ...
        if match_answer:
            ...
            if parse_order(answer) == sol['answer']:
                reward = 1.0
...
def tool_call_process_reward(target, messages, **kwargs):
    ...
    for sol, msg in zip(target, messages):
        ...
        # 每个标注窗口分别找到最匹配的预测窗口，计算IoU，除以标注窗口数量
        if len(target_time_windows) != len(predicted_time_windows):
            iou_score = 0.0
        else:
            iou_score_list = []
            for t, p in zip(target_time_windows, predicted_time_windows):
                iou_score_list.append(compute_iou([t], [p]))
            iou_score = sum(iou_score_list) / len(iou_score_list)
...
reward_functions = [
    ...
    sequence_answer_reward,
    time_iou_reward,
    # tool_call_process_reward,
]
