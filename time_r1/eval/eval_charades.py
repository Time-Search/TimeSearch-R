...
def parse_pred(msgs):
    ...
    answer = re.search(pattern_answer, msgs[-1]["content"][-1]["text"])
    if answer:
        answer = answer.group(1)
    else:
        answer = msgs[-1]["content"][-1]["text"]
    return answer

def main(prediction_file):
    datas = read_jsonl(prediction_file)
    ...
    ious = []
    for line in datas:
        target = line['target']['time']
        pred = line['prediction']
        pred = parse_pred(pred)
        # timestamps = extract_time(pred)
        try:
            timestamps = eval(pred)
            print(f"pred={pred},target={target},timestamps={timestamps}")
            # if len(timestamps) != 1:
                # print(f"pred={pred},timestamps={timestamps}")
                # timestamps = [[target[1] + 10, target[1] + 20]]
            # print(f"GT: {target}, Pred: {timestamps[0]}")
            ious.append(iou(target[0], timestamps[0]))
