...
from time_r1.eval.moment_retrieval_utils import (
...
def eval_llm_moment_retrieval(prediction_file, output_file=None, verbose=True):
    results = load_jsonl(prediction_file)
    ...
    results_interpreted = []
    for r in results:
        answer = re.search(pattern_answer, r["prediction"][-1]["content"][-1]["text"])
        if answer:
            answer = answer.group(1)
        else:
            answer = r["prediction"][-1]["content"][-1]["text"]
        new_r = {
            "qid": r["id"],
            "pred_relevant_windows": moment_str_to_list(answer),
            "relevant_windows": r["target"]["time"],
        }
        results_interpreted.append(new_r)
    print(results_interpreted[0])
...
if __name__ == "__main__":
    import fire
    fire.Fire(eval_llm_moment_retrieval)
